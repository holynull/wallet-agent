...
async function createOrder(params) {
    const API_ENDPOINT = 'https://api.omnibridge.pro/api/v2/accountExchange';

    // Validate required parameters
    const requiredParams = [
        'depositCoinCode', 'receiveCoinCode', 'depositCoinAmt',
        'destinationAddr', 'refundAddr'
    ];
    ...
    const payload = {
        ...
        depositCoinAmt: params.depositCoinAmt.toString(), // Ensure string
        receiveCoinAmt: params.receiveCoinAmt ? params.receiveCoinAmt.toString() : '0', // Default to 0 if not provided
        destinationAddr: params.destinationAddr,
        refundAddr: params.refundAddr,
        equipmentNo: params.equipmentNo || params.refundAddr, // Default to refundAddr
        sourceType: params.sourceType || 'H5', // Default to H5
        sourceFlag: params.sourceFlag || 'openclaw', // Default to openclaw
    };

    if (params.slippage) {
        payload.slippage = params.slippage.toString();
    }
